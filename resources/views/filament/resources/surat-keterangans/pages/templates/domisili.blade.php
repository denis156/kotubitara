@php
            ...
            // Data pemohon/pelapor - ambil dari penduduk atau manual
            ...
            // Data domisili
            $rt = $record->data_domisili['rt'] ?? ($record->penduduk?->kartuKeluarga?->rt ?? null);
            $rw = $record->data_domisili['rw'] ?? ($record->penduduk?->kartuKeluarga?->rw ?? null);
            $alamatLengkap = $record->data_domisili['alamat_lengkap'] ?? ($record->penduduk?->kartuKeluarga?->alamat ?? null);
            $lamaTinggal = $record->data_domisili['lama_tinggal'] ?? null;
            $statusTempatTinggal = $record->data_domisili['status_tempat_tinggal'] ?? null;

            // Match untuk status tempat tinggal
            $statusTempatTinggalLabel = match($statusTempatTinggal) {
...
<div class="mx-auto overflow-hidden bg-white" style="width: 210mm; height: 297mm; padding: 15mm 20mm; color: #000000 !important; font-family: 'Times New Roman', Times, serif;">
    ...
    <div class="mb-4 text-center">
        <h3 class="text-lg font-bold uppercase underline">Surat Keterangan Domisili</h3>
    ...
    <div class="mb-4 text-justify" style="font-size: 13px; line-height: 1.4;">
        ...
        <table class="mb-3 w-full">
            ...
            <tr>
                ...
                <td class="py-1">:</td>
                <td class="py-1">{{ $lamaTinggal }}</td>
            </tr>
            @endif
            @if($statusTempatTinggalLabel)
            <tr>
                <td class="py-1">Status Tempat Tinggal</td>
                <td class="py-1">:</td>
                <td class="py-1">{{ $statusTempatTinggalLabel }}</td>
            </tr>
            @endif
        </table>

        <p class="mb-3">Adalah benar warga yang berdomisili di Desa {{ $namaDesa }} pada alamat tersebut di atas.</p>

        @if($record->keperluan)
        <p class="mb-3">Surat keterangan domisili ini dibuat untuk keperluan: <strong>{{ $record->keperluan }}</strong></p>
        @endif
        ...
        <p class="mb-4">Demikian surat keterangan domisili ini dibuat dengan sebenarnya untuk dapat dipergunakan sebagaimana mestinya.</p>
    </div>
