@php
            ...
            // Data usaha
            $namaUsaha = $record->data_usaha['nama_usaha'] ?? null;
            $jenisUsaha = $record->data_usaha['jenis_usaha'] ?? null;
            $jenisUsahaLabel = match($jenisUsaha) {
            ...
            $alamatUsaha = $record->data_usaha['alamat_usaha'] ?? null;
            $lamaUsaha = $record->data_usaha['lama_usaha'] ?? null;
            $jumlahKaryawan = $record->data_usaha['jumlah_karyawan'] ?? null;
        @endphp
...
<div class="mx-auto overflow-hidden bg-white" style="width: 210mm; height: 297mm; padding: 15mm 20mm; color: #000000 !important; font-family: 'Times New Roman', Times, serif;">
    ...
    <div class="mb-4 text-center">
        <h3 class="text-lg font-bold uppercase underline">Surat Keterangan Usaha</h3>
    ...
    <div class="mb-4 text-justify" style="font-size: 13px; line-height: 1.4;">
        ...
        <p class="mb-3">Adalah benar warga Desa {{ $namaDesa }} yang memiliki usaha dengan keterangan sebagai berikut:</p>

        <table class="mb-3 w-full">
            @if($namaUsaha)
            <tr>
                <td class="w-1/3 py-1">Nama Usaha</td>
                <td class="w-8 py-1">:</td>
                <td class="py-1 font-semibold">{{ $namaUsaha }}</td>
            </tr>
            @endif
            @if($jenisUsaha)
            <tr>
                <td class="py-1">Jenis Usaha</td>
                <td class="py-1">:</td>
                <td class="py-1">{{ $jenisUsahaLabel }}</td>
            </tr>
            @endif
            @if($alamatUsaha)
            <tr>
                <td class="py-1" style="vertical-align: top;">Alamat Usaha</td>
                <td class="py-1" style="vertical-align: top;">:</td>
                <td class="py-1" style="vertical-align: top;">{{ $alamatUsaha }}</td>
            </tr>
            @endif
            @if($lamaUsaha)
            <tr>
                <td class="py-1">Lama Usaha</td>
                <td class="py-1">:</td>
                <td class="py-1">{{ $lamaUsaha }}</td>
            </tr>
            @endif
            @if($jumlahKaryawan)
            <tr>
                <td class="py-1">Jumlah Karyawan</td>
                <td class="py-1">:</td>
                <td class="py-1">{{ $jumlahKaryawan }} orang</td>
            </tr>
            @endif
        </table>

        <p class="mb-3">Usaha tersebut dijalankan di wilayah Desa {{ $namaDesa }} dan yang bersangkutan adalah warga yang baik dan taat hukum.</p>

        @if($record->keperluan)
        <p class="mb-3">Surat keterangan usaha ini dibuat untuk keperluan: <strong>{{ $record->keperluan }}</strong></p>
        @endif
        ...
        <p class="mb-4">Demikian surat keterangan usaha ini dibuat dengan sebenarnya untuk dapat dipergunakan sebagaimana mestinya.</p>
    </div>
