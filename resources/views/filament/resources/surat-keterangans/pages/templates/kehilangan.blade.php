@php
            ...
            // Data kehilangan
            $jenisBarangHilang = $record->data_tambahan['jenis_barang_hilang'] ?? null;
            $jenisBarangLabel = match($jenisBarangHilang) {
                'ktp' => 'KTP',
                'kk' => 'Kartu Keluarga',
                'sim' => 'SIM',
                'stnk' => 'STNK',
                ...
                'lainnya' => 'Lainnya',
                default => $jenisBarangHilang
            };
            $namaBarang = $record->data_tambahan['nama_barang'] ?? null;
            $tanggalKehilangan = $record->data_tambahan['tanggal_kehilangan'] ?? null;
            $tempatKehilangan = $record->data_tambahan['tempat_kehilangan'] ?? null;
            $kronologi = $record->data_tambahan['kronologi'] ?? null;
        @endphp
...
<div class="mx-auto overflow-hidden bg-white" style="width: 210mm; height: 297mm; padding: 15mm 20mm; color: #000000 !important; font-family: 'Times New Roman', Times, serif;">
    ...
    <div class="mb-4 text-center">
        <h3 class="text-lg font-bold uppercase underline">Surat Keterangan Kehilangan</h3>
    ...
    <div class="mb-4 text-justify" style="font-size: 13px; line-height: 1.4;">
        ...
        <p class="mb-3">Adalah benar warga Desa {{ $namaDesa }} yang melaporkan kehilangan dengan keterangan sebagai berikut:</p>

        <table class="mb-3 w-full">
            @if($jenisBarangHilang)
            <tr>
                <td class="w-1/3 py-1">Jenis Barang yang Hilang</td>
                <td class="w-8 py-1">:</td>
                <td class="py-1 font-semibold">{{ $jenisBarangLabel }}</td>
            </tr>
            @endif
            @if($namaBarang)
            <tr>
                <td class="py-1">Nama/Merk Barang</td>
                <td class="py-1">:</td>
                <td class="py-1">{{ $namaBarang }}</td>
            </tr>
            @endif
            @if($tanggalKehilangan)
            <tr>
                <td class="py-1">Tanggal Kehilangan</td>
                <td class="py-1">:</td>
                <td class="py-1">{{ \Carbon\Carbon::parse($tanggalKehilangan)->format('d F Y') }}</td>
            </tr>
            @endif
            @if($tempatKehilangan)
            <tr>
                <td class="py-1" style="vertical-align: top;">Tempat Kehilangan</td>
                <td class="py-1" style="vertical-align: top;">:</td>
                <td class="py-1" style="vertical-align: top;">{{ $tempatKehilangan }}</td>
            </tr>
            @endif
        </table>

        @if($kronologi)
        <p class="mb-3"><strong>Kronologi Kejadian:</strong></p>
        <p class="mb-3">{{ $kronologi }}</p>
        @endif

        <p class="mb-3">Yang bersangkutan adalah warga yang baik dan taat hukum.</p>

        @if($record->keperluan)
        <p class="mb-3">Surat keterangan kehilangan ini dibuat untuk keperluan: <strong>{{ $record->keperluan }}</strong></p>
        @endif
        ...
        <p class="mb-4">Demikian surat keterangan kehilangan ini dibuat dengan sebenarnya untuk dapat dipergunakan sebagaimana mestinya.</p>
    </div>
