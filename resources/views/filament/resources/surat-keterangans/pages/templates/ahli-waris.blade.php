@php
            ...
            // Data ahli waris
            $namaPewaris = $record->data_ahli_waris['nama_pewaris'] ?? null;
            $nikPewaris = $record->data_ahli_waris['nik_pewaris'] ?? null;
            $tanggalMeninggal = $record->data_ahli_waris['tanggal_meninggal'] ?? null;
            $hubunganDenganPewaris = $record->data_ahli_waris['hubungan_dengan_pewaris'] ?? null;
            $hubunganLabel = match($hubunganDenganPewaris) {
                ...
                'cucu' => 'Cucu',
                'lainnya' => 'Lainnya',
                default => $hubunganDenganPewaris
            };
            $keteranganHarta = $record->data_ahli_waris['keterangan_harta'] ?? null;
        @endphp
...
<div class="mx-auto overflow-hidden bg-white" style="width: 210mm; height: 297mm; padding: 15mm 20mm; color: #000000 !important; font-family: 'Times New Roman', Times, serif;">
    ...
    <div class="mb-4 text-center">
        <h3 class="text-lg font-bold uppercase underline">Surat Keterangan Ahli Waris</h3>
    ...
    <div class="mb-4 text-justify" style="font-size: 13px; line-height: 1.4;">
        ...
        <p class="mb-3">Adalah benar warga Desa {{ $namaDesa }} yang merupakan ahli waris sah dari:</p>

        <table class="mb-3 w-full">
            @if($namaPewaris)
            <tr>
                <td class="w-1/3 py-1">Nama Pewaris (Almarhum/Almarhumah)</td>
                <td class="w-8 py-1">:</td>
                <td class="py-1 font-semibold">{{ $namaPewaris }}</td>
            </tr>
            @endif
            @if($nikPewaris)
            <tr>
                <td class="py-1">NIK</td>
                <td class="py-1">:</td>
                <td class="py-1">{{ $nikPewaris }}</td>
            </tr>
            @endif
            @if($tanggalMeninggal)
            <tr>
                <td class="py-1">Tanggal Meninggal</td>
                <td class="py-1">:</td>
                <td class="py-1">{{ \Carbon\Carbon::parse($tanggalMeninggal)->format('d F Y') }}</td>
            </tr>
            @endif
            @if($hubunganDenganPewaris)
            <tr>
                <td class="py-1">Hubungan dengan Pewaris</td>
                <td class="py-1">:</td>
        ...
        @if($keteranganHarta)
        <p class="mb-3"><strong>Keterangan Harta Warisan:</strong></p>
        <p class="mb-3">{{ $keteranganHarta }}</p>
        @endif

        <p class="mb-3">Yang bersangkutan adalah ahli waris yang sah dan berhak atas warisan dari almarhum/almarhumah tersebut di atas.</p>

        @if($record->keperluan)
        <p class="mb-3">Surat keterangan ahli waris ini dibuat untuk keperluan: <strong>{{ $record->keperluan }}</strong></p>
        @endif
        ...
        <p class="mb-4">Demikian surat keterangan ahli waris ini dibuat dengan sebenarnya untuk dapat dipergunakan sebagaimana mestinya.</p>
    </div>
