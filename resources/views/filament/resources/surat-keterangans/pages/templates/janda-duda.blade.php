@php
            ...
            // Data janda/duda
            $status = $record->data_pernikahan['status'] ?? null;
            $statusLabel = match($status) {
                'janda' => 'Janda',
                'duda' => 'Duda',
                default => $status
            };
            $sebab = $record->data_pernikahan['sebab'] ?? null;
            $sebabLabel = match($sebab) {
                'cerai_mati' => 'Cerai Mati (Ditinggal Meninggal)',
                'cerai_hidup' => 'Cerai Hidup (Bercerai)',
                default => $sebab
            };
            $namaMantanPasangan = $record->data_pernikahan['nama_mantan_pasangan'] ?? null;
            $tanggalCeraiAtauMeninggal = $record->data_pernikahan['tanggal_cerai_atau_meninggal'] ?? null;
        @endphp
...
<div class="mx-auto overflow-hidden bg-white" style="width: 210mm; height: 297mm; padding: 15mm 20mm; color: #000000 !important; font-family: 'Times New Roman', Times, serif;">
    ...
    <div class="mb-4 text-center">
        <h3 class="text-lg font-bold uppercase underline">Surat Keterangan Janda/Duda</h3>
    ...
    <div class="mb-4 text-justify" style="font-size: 13px; line-height: 1.4;">
        ...
        @if($status)
        <p class="mb-3">Adalah benar warga Desa {{ $namaDesa }} yang saat ini berstatus <strong>{{ $statusLabel }}</strong> dengan keterangan sebagai berikut:</p>
        @else
        <p class="mb-3">Adalah benar warga Desa {{ $namaDesa }} yang saat ini berstatus janda/duda dengan keterangan sebagai berikut:</p>
        @endif

        <table class="mb-3 w-full">
            @if($namaMantanPasangan)
            <tr>
                <td class="w-1/3 py-1">Nama Mantan Pasangan</td>
                <td class="w-8 py-1">:</td>
                <td class="py-1">{{ $namaMantanPasangan }}</td>
            </tr>
            @endif
            @if($sebab)
            <tr>
                <td class="py-1">Sebab</td>
                <td class="py-1">:</td>
                <td class="py-1">{{ $sebabLabel }}</td>
            </tr>
            @endif
            @if($tanggalCeraiAtauMeninggal)
            <tr>
                <td class="py-1">Tanggal {{ $sebab === 'cerai_mati' ? 'Meninggal' : 'Cerai' }}</td>
                <td class="py-1">:</td>
                <td class="py-1">{{ \Carbon\Carbon::parse($tanggalCeraiAtauMeninggal)->format('d F Y') }}</td>
            </tr>
            @endif
        </table>
        ...
        @if($record->keperluan)
        <p class="mb-3">Surat keterangan janda/duda ini dibuat untuk keperluan: <strong>{{ $record->keperluan }}</strong></p>
        @endif
        ...
        <p class="mb-4">Demikian surat keterangan janda/duda ini dibuat dengan sebenarnya untuk dapat dipergunakan sebagaimana mestinya.</p>
    </div>
