@php
            ...
            // Data kelakuan baik
            $pekerjaan = $record->data_tambahan['pekerjaan'] ?? ($record->penduduk?->pekerjaan ?? null);
            $alamatLengkap = $record->data_tambahan['alamat_lengkap'] ?? ($record->penduduk?->kartuKeluarga?->alamat ?? null);
            $keperluanSurat = $record->data_tambahan['keperluan_surat'] ?? null;
            $keperluanSuratLabel = match($keperluanSurat) {
                ...
                'lainnya' => 'Lainnya',
                default => $keperluanSurat
            };
            $keteranganKelakuan = $record->data_tambahan['keterangan_kelakuan'] ?? 'Yang bersangkutan berkelakuan baik, tidak pernah terlibat tindak pidana, dan merupakan warga yang taat pada peraturan yang berlaku.';
        @endphp
...
<div class="mx-auto overflow-hidden bg-white" style="width: 210mm; height: 297mm; padding: 15mm 20mm; color: #000000 !important; font-family: 'Times New Roman', Times, serif;">
    ...
    <div class="mb-4 text-center">
        <h3 class="text-lg font-bold uppercase underline">Surat Keterangan Kelakuan Baik</h3>
    ...
    <div class="mb-4 text-justify" style="font-size: 13px; line-height: 1.4;">
        ...
        <table class="mb-3 w-full">
            ...
            <tr>
                ...
                <td class="py-1" style="vertical-align: top;">
                    @if($alamatLengkap)
                        {{ $alamatLengkap }}
                    @elseif($record->penduduk?->kartuKeluarga)
        ...
        <p class="mb-3">Adalah benar warga Desa {{ $namaDesa }}.</p>

        <p class="mb-3">{{ $keteranganKelakuan }}</p>

        @if($record->keperluan || $keperluanSurat)
        <p class="mb-3">Surat keterangan kelakuan baik ini dibuat untuk keperluan: <strong>{{ $record->keperluan ?? $keperluanSuratLabel }}</strong></p>
        @endif
        ...
        <p class="mb-4">Demikian surat keterangan kelakuan baik ini dibuat dengan sebenarnya untuk dapat dipergunakan sebagaimana mestinya.</p>
    </div>
