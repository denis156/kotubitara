@php
            ...
            // Data pernikahan
            $namaPasangan = $record->data_pernikahan['nama_pasangan'] ?? null;
            $tanggalMenikah = $record->data_pernikahan['tanggal_menikah'] ?? null;
            $tempatMenikah = $record->data_pernikahan['tempat_menikah'] ?? null;
            $nomorKutipanAktaNikah = $record->data_pernikahan['nomor_kutipan_akta_nikah'] ?? null;
        @endphp
...
<div class="mx-auto overflow-hidden bg-white" style="width: 210mm; height: 297mm; padding: 15mm 20mm; color: #000000 !important; font-family: 'Times New Roman', Times, serif;">
    ...
    <div class="mb-4 text-center">
        <h3 class="text-lg font-bold uppercase underline">Surat Keterangan Sudah Menikah</h3>
    ...
    <div class="mb-4 text-justify" style="font-size: 13px; line-height: 1.4;">
        ...
        <p class="mb-3">Adalah benar warga Desa {{ $namaDesa }} yang telah menikah dengan keterangan sebagai berikut:</p>

        <table class="mb-3 w-full">
            @if($namaPasangan)
            <tr>
                <td class="w-1/3 py-1">Nama Pasangan</td>
                <td class="w-8 py-1">:</td>
                <td class="py-1 font-semibold">{{ $namaPasangan }}</td>
            </tr>
            @endif
            @if($tanggalMenikah)
            <tr>
                <td class="py-1">Tanggal Menikah</td>
                <td class="py-1">:</td>
                <td class="py-1">{{ \Carbon\Carbon::parse($tanggalMenikah)->format('d F Y') }}</td>
            </tr>
            @endif
            @if($tempatMenikah)
            <tr>
                <td class="py-1">Tempat Menikah</td>
                <td class="py-1">:</td>
                <td class="py-1">{{ $tempatMenikah }}</td>
            </tr>
            @endif
            @if($nomorKutipanAktaNikah)
            <tr>
                <td class="py-1">No. Kutipan Akta Nikah</td>
                <td class="py-1">:</td>
                <td class="py-1">{{ $nomorKutipanAktaNikah }}</td>
            </tr>
            @endif
        </table>
        ...
        @if($record->keperluan)
        <p class="mb-3">Surat keterangan sudah menikah ini dibuat untuk keperluan: <strong>{{ $record->keperluan }}</strong></p>
        @endif
        ...
        <p class="mb-4">Demikian surat keterangan sudah menikah ini dibuat dengan sebenarnya untuk dapat dipergunakan sebagaimana mestinya.</p>
    </div>
